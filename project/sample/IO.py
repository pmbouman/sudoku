...
def euler_init_allowable_set(cellchar):
    """
    Solver configured tp read from CSV with an entry
    for each cell (1:9) or a blank
    This creates the set of possible walues in puzzle.
...
def _euler_readfile(f):
    readin_puzzle = {}

    f.readline()    ## skip the grid names
    for rowname in c.ROWNAMES:
        puzzle_row = f.readline()
        row_entries = list(puzzle_row.rstrip())

        for i in range(1, 10):
            cellname = rowname + str(i)
            entry = euler_init_allowable_set(row_entries[i-1])
            readin_puzzle[cellname] =\
                {"Marked?": "Unmarked", "Allowable": entry}
    return readin_puzzle


def euler_doall(fname, s):   ### s a solver routine to run
    f = open(fname)
    for i in range(1, 51):
        print("grid ",i)
        s(_euler_readfile(f))
